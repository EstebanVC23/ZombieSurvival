...
# ===============================
# IMPORTAR TODAS LAS CONSTANTES DEL SETTINGS
# ===============================
...
class Zombie(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, ztype="common", level=1, rarity=None):
        ...
        self.radius = stats["radius"]  # NO CAMBIAR tamaño
        ...
        # ===============================
        if self.level > 1:
            level_multiplier = 1 + (self.level - 1) * 0.15  # +15% por cada nivel extra
            self.hp = int(self.hp * level_multiplier)
            self.damage = int(self.damage * level_multiplier)
            self.speed *= level_multiplier

        # Puntaje según rareza
        self.score_value = int(ZOMBIE_SCORE_VALUES[self.type] * ZOMBIE_RARITY_SCORE_MULT[self.rarity])

        # Chance extra de drop
    ...
    def update(self, dt, game):
        if self.dead:
            ...
            if self.dead_timer >= 4.0:
                self.kill()  # lo elimina del grupo de sprites
            return  # <- Aquí ya no hace nada más, colisiones ignoradas
        ...
        # Movimiento y colisión con jugador
    ...
    def take_damage(self, dmg, game=None):
        if self.dead:
            return  # <- Ignorar balas si ya está muerto
        ...
        # Evitar que las balas colisionen
        self.radius = 0
